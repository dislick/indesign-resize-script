...
if (mFramesToMove.length) {
  if (myDoc.layers.item('Overset frames').isValid)
    ...
    mLayer = myDoc.layers.add({
      LocationOptions: LocationOptions.AT_BEGINNING,
      name: 'Overset frames'
    });
  ...
  alert(
    'Some frames (' +
      mFramesToMove.length +
      ") stay overset\rTake a look at layer 'Overset Frames' "
  );
} else alert(count + ' frames overset fixed');

function mDialog(number) {
  // "What to do" dialog
  var w = new Window('dialog', 'What to do?', undefined, {
    closeButton: false
  });
  w.add(
    'statictext',
    undefined,
    'Number of overset texts found:  ' + number.toString()
  );
  ...
  if (w.show() == 1) return [Ch1.value, Ed1.text, Ch2.value, Ed2.text];
  else exit();
}

function mShrink(textFr, ToDoArr) {
  // input textFrame and array with mDialog result
  var pSizeLimAc,
    pSizeLim,
    pSizeCurr,
    pSizeDiff,
  ...
  pSizeLim = ToDoArr[1];
  if (pSizeLimAc) {
    if (isNaN(parseFloat(pSizeLim)))
      return [false, 'point size is not a number'];
  ...
  hScaleLim = ToDoArr[3];
  if (hScaleAc) {
    if (isNaN(parseFloat(hScaleLim)))
      return [false, 'hori scale is not a number'];
